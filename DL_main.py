...
# st.write("Pick a date to uncover past trends with real prices or explore future forecasts ahead!")
...
# #Date input
# user_date = st.date_input("Select Date", date.today() + timedelta(days=1))
...
#             st.info(f"📅 Date: {user_date}")
#             st.write(f"🔹 Predicted Google stock price : ${predicted_price:.2f}")

#         #PAST Prediction 
#         elif user_date <= date.today():
#             try:
#                 #Find index of selected date
#                 idx = data.index[data['Date'] == pd.to_datetime(user_date)][0]

#                 #Ensure we have at least 60 days before that date
#                 if idx < 60:
#                     st.error("Not enough historical data before this date to make prediction.")
#                 else:
...
#                     st.success("✅ Prediction Completed!")
#                     st.info(f"📅 Date: {user_date}")
#                     st.write(f"🔹 Actual Google stock Price : ${actual_price:.2f}")
#                     st.write(f"🔹 Predicted Google stock Price : ${predicted_price:.2f}")

#             except IndexError:
#                 st.error("Selected past date not found in dataset.")
